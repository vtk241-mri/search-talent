...
export default function PublicProfileShowcase({
  ...
  const sectionMap = new Map<ProfileSectionId, { title: string; content: ReactNode; visible: boolean }>([
    ...
    ["projects", { title: dictionary.creatorProfile.projects, visible: true, content: projects.length > 0 ? <><p className="text-sm leading-7 app-muted">{dictionary.creatorProfile.publishedWork}</p><div className="mt-5 grid gap-4 md:grid-cols-2">{projects.map((project) => <ProjectCard key={project.id} dictionary={dictionary} project={{ ...project, slug: project.slug || "", ownerName: profile.name, ownerUsername: profile.username }} />)}</div></> : <p className="text-sm app-muted">{dictionary.creatorProfile.noProjects}</p> }],
  ]);
